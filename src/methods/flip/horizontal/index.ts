...
const getBackgroundStyle: StyleGetter = () => (`
  :host {
    ${Style.rightXLeft}: 50%;
    ${Style.rightXRight}: 100vw;
    ${Style.leftXLeft}: -100vw;
    ${Style.leftXRight}: 50%;

    ${prefixCss(`clip-path: polygon(
      var(${Style.xLeft}) -100vh,
      var(${Style.xRight}) -100vh,
      var(${Style.xRight}) calc(100% + 100vh),
      var(${Style.xLeft}) calc(100% + 100vh)
    );`, 'webkit')}
  }
`);
...
const getCandidateStyle: StyleGetter = (options) => (`
  ...
  :host {
    --hori-perspective: ${options.perspective};
    --right-x-left: 50%;
 ...
 */
function flippingHorizontally(options: FlipAnimationOption): Promise<void> {
  const {
    duration,
    direction,
    lastCandidateInfo,
